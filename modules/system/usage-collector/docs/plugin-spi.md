...
- [ModKit Plugin SPI placement](#modkit-plugin-spi-placement)
...
taxonomy, ModKit crate placement, versioning and stability policy,
...
under `modules/system/usage-collector/plugins/<backend>/`. Each
...
- The canonical trait name is `UsageCollectorPluginV1`, mirroring the
  ...
  §3.12.9 and the ModKit naming convention that places the module
...
- Every method takes `&self` as the receiver and accepts only the
  ...
  `modules/system/authn-resolver/authn-resolver-sdk/src/plugin_api.rs:31-55`,
  and `modules/system/authz-resolver/authz-resolver-sdk/src/plugin_api.rs:19-22`,
...
#[gts_type_schema(
    dir_path = "schemas",
    base = PluginV1,
    schema_id = "gts.cf.modkit.plugins.plugin.v1~cf.core.usage_collector.plugin.v1~",
...
Each `usage-collector-plugin-<backend>` crate's `#[modkit::module]`
...
`gts.cf.modkit.plugins.plugin.v1~cf.core.usage_collector.plugin.v1~<vendor>.<package>.usage_collector_plugin.v1`).
...
workspace member and registered with ModKit via `#[modkit::module]`
...
`libs/modkit-gts/src/plugin.rs`). The resolved instance is cached
...
`parking_lot::RwLock` (see `libs/modkit/src/client_hub.rs:155-165`),
...
`authz-resolver` for the canonical pattern; `libs/modkit-gts/src/plugin.rs`
...
- `UsageRecordRow` — the per-row payload Method 4 emits. Carries every
  ...
  `modkit_odata::Page<UsageRecord>`; semantically a `UsageRecord`,
...
- `UsageRecordFilterField`
  ...
  module-owned Rust enum implementing `modkit_odata::filter::FilterField`.
...
ModKit gateway (`modkit_odata::CursorV1` plus
`modkit_odata::validate_cursor_against`); plugins do not mint,
...
- The plugin is the authority for `id` allocation on accepted
  ...
  gateway-owned (`modkit_odata::CursorV1` plus
...
- Cursor lifecycle (gateway-owned): cursor decode, structural
  ...
  `modkit_odata::validate_cursor_against` happen at the gateway
...
`cpt-cf-usage-collector-nfr-batch-and-report-timing`;
`research-modkit-alignment.md` §1 D10 (gateway-owned cursor lifecycle,
...
NOT depend on `modkit-canonical-errors`**; plugin authors and the
...
itself never sees `modkit-canonical-errors`.
...
- CORS, TLS termination, output encoding, and HTTP-level rate
  ...
  §3.9.3.
- Platform liveness and readiness probes are handled by the ModKit host above the module boundary; the collector exposes no module-local health endpoints. Operational telemetry is pushed via OTLP from ModKit's global `SdkMeterProvider` (no in-module `/metrics` scrape endpoint exists). The SPI contributes no `ready` or `flush` operation; the structural readiness fact (selector cached AND `ClientHub::try_get_scoped` returns `Some`) is composed by the Plugin Host and surfaced via the `usage_collector.plugin.ready` gauge.
...
- `cpt-cf-usage-collector-entity-usage-record`,
  ...
  `modkit_odata::Page<UsageRecord>` shape; cursor lifecycle is
  realized by `modkit_odata::CursorV1` plus
...
- OQ-4 — Whether the SPI surfaces a separate `health` probe distinct
  ...
  the Plugin Host's process-level health, observed by the ModKit
