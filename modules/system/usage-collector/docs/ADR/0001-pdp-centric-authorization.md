...
- `cpt-cf-usage-collector-nfr-authentication` — every API operation must arrive with a `SecurityContext` resolved by the ModKit gateway upstream of the collector before any business processing.
...
PDP enforcement is performed **per domain component**, not via a centralized adapter. Each domain component — ingestion-gateway, query-gateway, deactivation-handler, and metric-catalog — accepts `ctx: &SecurityContext` as the first parameter on every operation and calls `authz-resolver-sdk`'s `PolicyEnforcer::access_scope_with(ctx, ...)` inline through a thin shared helper. The reference pattern is the `authz_scope` helper in `modules/system/account-management/account-management/src/domain/authz.rs`, which collapses the per-service authorization surface into a single definition site while keeping the call site local to each domain component. The collector defines an equivalent helper and reuses it across its four domain components.

Authentication is owned upstream by the ModKit gateway (REST surface) via `OperationBuilder::authenticated()` or supplied directly by the in-process caller (SDK trait surface). The collector accepts only a pre-resolved `SecurityContext` and never synthesizes identity, never resolves credentials, and never consumes a platform AuthN contract. Any operation arriving without a resolved `SecurityContext` is rejected at the surface boundary before any domain component runs.
...
- Locality and account-management parity: PDP enforcement lives next to the operation it guards inside each domain component, matching the established `authz_scope` pattern in `modules/system/account-management/account-management/src/domain/authz.rs`. Each component's helper invocation is reviewable in isolation alongside the business logic it gates.
...
- `cpt-cf-usage-collector-nfr-authentication` — every operation enters with a `SecurityContext` resolved by the ModKit gateway upstream before any handler executes; the collector itself never resolves credentials.
