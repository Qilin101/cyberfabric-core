...
- Signed compensation entry on the unified ingestion path — the source module emits an entry with `entry_type=compensation`, a negative `value`, and `corrects_id` pointing at the original `usage` row, through the existing emit endpoint / SDK method / SPI persist call. Same PDP attribution, same mandatory idempotency key, same validation lane (extended by `(kind × entry_type)` matrix), same storage row shape.
...
The source module emits an `entry_type=compensation` row with a negative `value` and `corrects_id`, through the same emit endpoint / SDK method / SPI persist call already in use for `entry_type=usage`.
