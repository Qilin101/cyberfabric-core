...
- [ModKit SDK placement](#modkit-sdk-placement)
...
domain types, error taxonomy, ModKit placement, versioning and stability
...
REST-only). Operational telemetry is pushed via OTLP from ModKit's
...
aggregation pushdown — authentication is owned by the ModKit gateway
...
Sources: phase-02 §"ModKit Conventions" / "Crate placement and layout";
...
- The canonical trait name is `UsageCollectorClientV1`, following the
  ModKit naming convention that places the module name and capability
...
Sources: phase-02 §"ModKit Conventions" / "Trait shape", "ClientHub
...
Sources: phase-02 §"ModKit Conventions" / "Trait shape" two-trait
...
§"Query Models And Views"; phase-02 §"ModKit Conventions" / "Domain
types and models".
...
- `SecurityContext` (`cpt-cf-usage-collector-entity-security-context`).
  Resolved platform caller identity supplied by the ModKit gateway
...
- `RawQuery` (`cpt-cf-usage-collector-entity-raw-query`). The SDK
  ...
  - `metric_gts_id` — GTS metric identifier; **REQUIRED** per
    ...
    surface.
  - `filter_ast: modkit_odata::filter::FilterNode<UsageRecordFilterField>`
  ...
  - `order: modkit_odata::ODataOrderBy` — parsed `$orderby`; for raw
    queries it is the canonical `timestamp asc, id asc` order.
  - `page_after: Option<modkit_odata::Keyset>` — typed `(timestamp,
...
- `Keyset` (`cpt-cf-usage-collector-entity-keyset`). Typed
  `(timestamp, id)` tuple consumed by the modkit cursor encoder; the
...
limit }` where `next_cursor` / `prev_cursor` are opaque
  `CursorV1` envelopes minted by the modkit gateway from the
  plugin-returned last-row `Keyset`.

Cursor envelopes are opaque to SDK callers and are minted, decoded,
and validated by the modkit gateway (`modkit_odata::CursorV1` plus
`modkit_odata::validate_cursor_against`). SDK callers pass
...
`out/phase-01-domain-contracts.md` §§1–4; `research-modkit-alignment.md`
...
- `PageParams` — keyset-pagination input for `list_metrics`:
  ...
  plugin dispatch.
- `Page<T>` — the platform `modkit_odata::Page<T>` envelope reused
...
- Every read and write operation requires a resolved `SecurityContext`
  (resolved by the ModKit gateway upstream of the collector on the
...
| Raw keyset-paginated query   | `fr-query-raw`, `seq-query-raw`                                                                            | One `ODataQuery<UsageRecordFilterField>` (parsed `filter_ast`, `order`, optional `page_after`, `limit`).                                                                                                                                                                                                  | `modkit_odata::Page<UsageRecord>` (`items` + `page_info` with opaque `CursorV1`).                                                                                                                                                                                                                                                                                               |
...
Note on `SecurityContext` parameter placement: phase-02 §"ModKit
...
`research-modkit-alignment.md` and is anchored at the per-domain
...
Sources: `research-modkit-alignment.md` §1 D11 (PDP enforcement
...
- Validation behaviour (executed in order before plugin dispatch):
  1. Missing or invalid `SecurityContext` (authentication is owned by
     the ModKit gateway upstream of the collector) yields the
...
- Validation behaviour:
  1. Missing/invalid `SecurityContext` (authentication owned by the
     ModKit gateway upstream) yields the `Authentication` error
...
- Canonical Rust signature:
  ...
  ```rust
  pub struct ODataQuery<F: modkit_odata::filter::FilterField> {
      pub filter_ast: modkit_odata::filter::FilterNode<F>,
      pub order:      modkit_odata::ODataOrderBy,
      pub page_after: Option<modkit_odata::Keyset>,
      pub limit:      u64,
  }
  ```
...
- Validation behaviour (executed by the trait implementation, in
  ...
  5. Plugin-side failures map to `PluginUnavailable`, `PluginTimeout`,
     or `PluginFailure`.
- Success output: `modkit_odata::Page<UsageRecord>` with `items`
...
§3–§4; `research-modkit-alignment.md` §1 D9 (SDK trait return type),
...
- Structural inputs: `ListMetricsFilter { kind }` (see
  ...
  the gateway via `modkit_odata::CursorV1`).
...
`modkit-canonical-errors`**; consumers pattern-match variants of
...
`From<UsageCollectorError> for modkit_canonical_errors::CanonicalError`
...
- `ServiceUnavailable` — Non-plugin transient infrastructure failure;
  ...
  is owned by `modkit-canonical-errors`' `IntoResponse` and is not
...
And Problem Mapping", §"SDK Method Inputs And Outputs", §"ModKit
...
- Platform liveness and readiness probes (handled by the ModKit host above the module boundary — the collector does not expose module-local health endpoints).
- Operational telemetry; instruments are pushed via OTLP from
  ModKit's global `SdkMeterProvider` (no in-module HTTP metrics
...
- `cpt-cf-usage-collector-entity-usage-record`,
  ...
  output is the canonical `modkit_odata::Page<UsageRecord>`
  envelope; cursor lifecycle is realized by
  `modkit_odata::CursorV1` plus `validate_cursor_against`, per
...
- OQ-2 (phase-02 OQ-2, gap G-10): Whether `IdempotencyKey` is a
  ...
  newtype in `models.rs`, consistent with the ModKit `models.rs`
...
- OQ-5 (gap G-8): Whether `correlation_id` on `SecurityContext` is
  ...
  provided by the ModKit gateway upstream of the collector (REST) or
...
§"ModKit Conventions" / "Trait shape" V1-suffix bullet, §"SDK Method
