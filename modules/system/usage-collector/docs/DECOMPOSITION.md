...
- [2. Entries](#2-entries)
  - [2.1 Module Foundation & Pluggable Storage ⏳ HIGH](#21-module-foundation--pluggable-storage--high)
...
- **Scope**:
  ...
  - Plugin host lifecycle: at `Module::init` the host reads `[usage_collector].vendor` once via `ctx.config_or_default()?` and constructs the `Service` with an embedded `GtsPluginSelector` (no `types-registry` query yet); each `usage-collector-plugin-<backend>` `init()` independently registers its scoped `dyn UsageCollectorPluginV1` in `ClientHub` under `ClientScope::gts_id(&instance_id)`; on the first dispatch after the `types-registry` is consistent the host lazily resolves the bound instance via `GtsPluginSelector::get_or_init` (single-flight, cached for the `Service`'s lifetime) and looks the client up via `ClientHub::try_get_scoped`. There is no separate "Module Orchestrator" component — binding is decentralised across the host module's `Service` constructor and each plugin module's own `init()`. Binding changes require a module restart; there is no runtime configuration-change channel.
  ...
  - REST API contract surface (`cpt-cf-usage-collector-interface-rest-api`) registration behind the platform API gateway and SDK trait surface (`cpt-cf-usage-collector-interface-sdk-client`) registration in ClientHub for in-process consumers. Operational telemetry is pushed via OTLP from ModKit's global meter provider; no module-local Prometheus-scrape endpoint and no module-local health endpoints are exposed (platform liveness and readiness are handled by the ModKit host).
  - Deployment topology (`cpt-cf-usage-collector-topology-module-runtime`): stateless, horizontally scaled instances behind the platform API gateway with durable state reached exclusively through the ClientHub-bound plugin.
...
- **Data**:
  - [ ] `p3` - `cpt-cf-usage-collector-db-module-store`
...
- **Scope**:
  ...
  - PDP-gated operator authority: every Metric register and Metric delete call receives an already-resolved caller `SecurityContext` (populated upstream by the ModKit gateway on REST or supplied directly to the SDK) and authorizes the mutation inline through the per-component PDP authorization helper against `cpt-cf-usage-collector-contract-authz-resolver` before any Plugin SPI call is dispatched.
  ...
  - **Dimension-aware query path work-package** — per `cpt-cf-usage-collector-adr-0012-unified-plugin-catalog-and-gts-id-reference` Amendment 2026-06-02 (consumed by [§2.4](#24-usage-query--medium) Usage Query): `RawQuery.metric_gts_id` is **REQUIRED** (no longer optional) so the declared-key set can be resolved per request; `AggregationQuery.group_by` is **fixed-fields plus per-metric declared metadata fields** (fixed fields: `tenant`, `resource`, `subject`, `source_module`, `timestamp`, `status`); `$filter` accepts the queried metric type's declared metadata keys on top of fixed fields. Declared keys are resolved per request from the queried metric's `metadata_fields` list in the catalog row (cached as `HashSet<String>` in the L1 catalog cache); there are no undeclared "extras" — undeclared keys are rejected at ingest. Cross-metric aggregation is out of scope (single-metric aggregation is required for declared-key resolution).

- **Out of scope**:
  - Source-module-to-Metric emit authorization — owned by the PDP as operator-managed policy, not stored inside the Usage Collector.
  - Per-Metric business-rule validation, accounting / billing semantics, or pricing — owned by source modules and downstream consumers, never by the catalog. (Note: the catalog DOES carry typed declared-dimension validation per ADR 0012, which is a metadata shape constraint, not business logic.)
...
- **Out of scope**:
  ...
  - Plugin host lifecycle, shared PDP authorization helper definition, REST/SDK/Plugin SPI surface declaration, and deployment topology — owned by [§2.1](#21-module-foundation-pluggable-storage-high) Module Foundation & Pluggable Storage.
  ...
  - Module-local audit-ledger emission for accepted records — authoritative audit is delegated to the platform gateway access log and PDP decision logs per `cpt-cf-usage-collector-fr-audit-trail`; a dedicated in-module audit-emission capability is deferred per DESIGN §3.9.5 and [§4](#4-crate-layout-platform-dependencies).
...
- **Scope**:
  - Aggregated query path (`POST /usage-collector/v1/records/aggregate`) — mandatory time range and single-Metric filter, optional tenant / subject / resource / source-module filters, server-side SUM / COUNT / MIN / MAX / AVG with grouping pushed into the bound storage plugin via the Plugin SPI. `AggregationQuery.group_by` is **fixed fields plus per-metric declared metadata keys** per `cpt-cf-usage-collector-adr-0012-unified-plugin-catalog-and-gts-id-reference` Amendment 2026-06-02; fixed fields are `tenant`, `resource`, `subject`, `source_module`, `timestamp`, `status`; dynamic fields are the queried metric type's declared keys from `metadata_fields` resolved via the L1 catalog cache produced by [§2.2](#22-metric-catalog--lifecycle--high). Cross-metric aggregation is out of scope — single-metric is required so the declared-key set is unambiguous.
  - Raw query path (`GET /usage-collector/v1/records`) — `RawQuery.metric_gts_id` is **REQUIRED** per `cpt-cf-usage-collector-adr-0012-unified-plugin-catalog-and-gts-id-reference` (was optional pre-rework); OData query parameters `$filter` (mandatory time range plus optional narrowing on fixed fields and the queried metric type's declared metadata keys), `$orderby` (must be a prefix of the canonical keyset `(timestamp, id)`), `$top` (page size, server-clamped to `[1, 1000]`), and `cursor` (modkit cursor encoded by the gateway over the standardized sort keyset). The declared-key set the `$filter` AST is type-checked against is resolved per request from the queried metric's `metadata_fields` list served by the L1 catalog cache; there are no undeclared "extras" — undeclared keys are rejected at ingest, so every row only carries declared keys. Cursor decode and validate-against-current-`$filter`/`$orderby` happen at the gateway via `modkit_odata::validate_cursor_against`; the plugin is dispatched with a structured `(filter_ast, order, page_after, limit)` tuple and returns `(rows, last_keyset)` from which the gateway re-issues the next cursor. The response envelope is `modkit_odata::Page<UsageRecord>`.
...
- **Requirements Covered**:
  ...
  - [ ] `p1` - `cpt-cf-usage-collector-fr-tenant-isolation`
  - [ ] `p1` - `cpt-cf-usage-collector-fr-data-ownership` (cross-cutting governance FR; primary owner is [§2.1](#21-module-foundation--pluggable-storage--high) Foundation — listed here because Usage Query realizes the read-path data-sharing constraint)
...
- **Domain Model Entities**:
  ...
  - UsageRecordFilterField (`cpt-cf-usage-collector-entity-usage-record-filter-field`) — fixed-field set bounded by the Rust enum; dynamic per-metric declared dimensions admitted on top per `cpt-cf-usage-collector-adr-0012-unified-plugin-catalog-and-gts-id-reference`; implementing the `modkit_odata::filter::FilterField` contract carries the static portion of the OData `$filter` AST accepted by the raw query path.
  - Keyset (`cpt-cf-usage-collector-entity-keyset`) — canonical `(timestamp, id)` sort tuple consumed by the modkit cursor envelope for raw-read pagination.
  - PdpConstraint (`cpt-cf-usage-collector-entity-pdp-constraint`)
  - SecurityContext (`cpt-cf-usage-collector-entity-security-context`)
  - ResourceRef (`cpt-cf-usage-collector-entity-resource-ref`)
...
- **Scope**:
  ...
  - Audit-trail correlation: a request-level correlation identifier (per `cpt-cf-usage-collector-fr-audit-trail`) is propagated through the deactivation flow so platform gateway and PDP decision logs can be reconciled with module-level activity.
...
- **Out of scope**:
  ...
  - Module-local audit event emission for the deactivate operation — owned by the platform gateway access log and PDP decision logs per `cpt-cf-usage-collector-fr-audit-trail`; per-record audit-ledger emission inside the module is explicitly deferred.
...
- **Out of scope**:
  ...
  - Computing refunds, credits, credit-notes, or quota balances — explicitly owned by downstream consumers; the Usage Collector records what the source module decides to apply, never computes one itself, per `cpt-cf-usage-collector-constraint-no-business-logic`.
  ...
  - Plugin host lifecycle, REST / SDK / Plugin SPI surface declaration, and deployment topology — owned by [§2.1](#21-module-foundation-pluggable-storage-high) Module Foundation & Pluggable Storage.
...
The following `cpt-cf-usage-collector-*` IDs from the element inventory are intentionally not assigned to any [§2.1](#21-module-foundation-pluggable-storage-high)..[§2.6](#26-compensation--medium) feature. Each omission is justified by either the kind being a non-implementation artifact (role descriptions, section anchors, PRD-side abstractions) or by an explicit scope boundary stated in DESIGN/PRD.

- `cpt-cf-usage-collector-actor-platform-developer`: PRD-side role description for the platform developer audience; module surfaces (SDK trait, REST, Plugin SPI) are owned by [§2.1](#21-module-foundation-pluggable-storage-high) Foundation, no actor-specific implementation artifact required.
- `cpt-cf-usage-collector-actor-platform-operator`: PRD-side role description for the operator audience; operator authority is enforced via the shared PDP authorization helper owned by [§2.1](#21-module-foundation-pluggable-storage-high), with concrete operator flows (metric register/delete, deactivate) already covered by [§2.2](#22-metric-catalog-lifecycle-high) and [§2.5](#25-event-deactivation-medium).
- `cpt-cf-usage-collector-actor-storage-backend`: PRD-side role description for storage-vendor implementors; the Plugin SPI contract surface they implement is owned by [§2.1](#21-module-foundation-pluggable-storage-high) Foundation.
...
- `cpt-cf-usage-collector-actor-usage-source`: PRD-side role description for source modules emitting usage; the source-module write surface is owned by [§2.3](#23-usage-emission-high) Usage Emission.
- `cpt-cf-usage-collector-design-usage-collector`: Top-level DESIGN.md section anchor; its constituent design elements (components, sequences, principles, entities) are each individually assigned to features [§2.1](#21-module-foundation-pluggable-storage-high)..[§2.5](#25-event-deactivation-medium).
- `cpt-cf-usage-collector-design-security-architecture`: DESIGN.md section anchor for the security architecture; constituent principles (`principle-fail-closed`, `principle-pdp-centric-authorization`) and the shared PDP authorization helper definition are owned by [§2.1](#21-module-foundation-pluggable-storage-high) Foundation.
- `cpt-cf-usage-collector-design-reliability-data-architecture`: DESIGN.md section anchor for reliability/data architecture; constituent storage components and data entities are owned by [§2.1](#21-module-foundation-pluggable-storage-high) (Plugin SPI, db-module-store) and [§2.3](#23-usage-emission-high) / [§2.4](#24-usage-query-medium) / [§2.5](#25-event-deactivation-medium) (usage_records).
- `cpt-cf-usage-collector-design-performance-operations-architecture`: DESIGN.md section anchor for performance/operations architecture; constituent NFRs (throughput, latency, workload-isolation, operational-visibility) are owned by [§2.1](#21-module-foundation-pluggable-storage-high) Foundation and [§2.3](#23-usage-emission-high) Usage Emission.
- `cpt-cf-usage-collector-design-maintainability-testing-ux-integration`: DESIGN.md section anchor for maintainability/testing/UX/integration architecture; constituent NFRs (developer-operator-experience, documentation-coverage, error-experience) are owned by [§2.1](#21-module-foundation-pluggable-storage-high) Foundation.
- `cpt-cf-usage-collector-design-capacity-cost-compliance-doc-architecture`: DESIGN.md section anchor for capacity/cost/compliance/doc architecture; constituent NFRs (capacity-headroom, scalability) are owned by [§2.1](#21-module-foundation-pluggable-storage-high) Foundation and the explicit compliance non-goal is captured below.
...
- `cpt-cf-usage-collector-constraint-pii-identity-layer`: Explicit out-of-scope marker stating PII handling is owned by the platform identity layer, not the Usage Collector; no implementation in this module.
- `cpt-cf-usage-collector-constraint-regulatory-out-of-scope`: Explicit out-of-scope marker stating regulatory (HIPAA/PCI/etc.) compliance is owned by downstream consumers and the platform, not the metering substrate; no implementation in this module.
...
The Usage Collector ships exactly two first-party crates following the platform-standard `<module>` + `<module>-sdk` two-crate layout used by every reference module (`credstore`, `authn-resolver`, `authz-resolver`). There is no separate `-contracts` crate and no separate `-plugin-api` crate: the consumer SDK trait, the plugin trait, the GTS spec for plugin discovery, the domain models, and the public error enum all live inside the single `usage-collector-sdk` crate alongside each other.

### 4.1 Two-crate layout

- `usage-collector-sdk` (public contract crate):
  - Purpose: public contract surface consumed in-process by source modules and downstream readers AND by plugin authors. Single source of truth for the SDK trait, the Plugin trait, the GTS spec, the domain models, and the public error enum.
  - File layout under `src/`:
    - `api.rs` — `UsageCollectorClientV1` trait (consumer SDK trait; what modules call via ClientHub).
...
- `usage-collector` (host module crate):
  - Purpose: REST machinery, module wiring, plugin resolution, and the in-process implementation of the SDK trait.
  - File layout under `src/`:
    - `module.rs` — `#[modkit::module]` entrypoint, ClientHub wiring, REST route registration.
...
- Concrete-plugin crates (out of scope to implement here; the spec only describes how they plug in): one per backend under `modules/system/usage-collector/plugins/<backend>/` (e.g. `usage-collector-plugin-clickhouse`, `usage-collector-plugin-timescaledb`), depend on `usage-collector-sdk` only — never on the host crate — and are compiled in at the workspace level.

### 4.2 Direct platform dependencies

The crates depend directly on the following ModKit platform crates (existing edges to storage plugins, the `authz-resolver` consumer SDK (per `cpt-cf-usage-collector-contract-authz-resolver`), the GTS Registry contract, and the runtime are preserved unchanged):

- `usage-collector-sdk` (public contract crate):
  - `modkit` — ModKit core building blocks consumed by the SDK and Plugin traits.
  - `modkit-gts` and `gts`, `gts-macros` — GTS spec macros and runtime used to declare the plugin discovery type system.
  - `modkit-security` — `SecurityContext` and related security primitives surfaced through the trait signatures.
  ...
  - `modkit-odata` (optional) — for `Page<T>` re-export and the `FilterField` derive on `UsageRecordFilterField` consumed by the Plugin trait.

  The SDK crate does **NOT** depend on `modkit-canonical-errors`. Consumers pattern-match `UsageCollectorError` variants directly; the lift to `modkit_canonical_errors::CanonicalError` lives in the host crate at `usage-collector/src/infra/sdk_error_mapping.rs`. This mirrors the platform standard set by `account-management-sdk`, `credstore-sdk`, `authn-resolver-sdk`, and `authz-resolver-sdk`: SDK crates publish a flat module-specific error enum (via `thiserror::Error`) and never take a dependency on the canonical-errors envelope crate.

- `usage-collector` (host module crate):
  - `usage-collector-sdk` — the public contract crate (path dependency).
  - `modkit` — ModKit core building blocks for module wiring and REST registration.
  - `modkit-canonical-errors` — provides the canonical `Problem` error envelope. The host crate's `infra/sdk_error_mapping.rs` lifts `UsageCollectorError` (from the SDK) into `modkit_canonical_errors::CanonicalError`, whose built-in `IntoResponse` produces the RFC-9457 `Problem` response. The module-specific `context.reason` discriminator names (`cursor_decode`, `order_mismatch`, `filter_mismatch`, `unknown_metric`, `kind_invariant`, …) are produced by the host's lift from `UsageCollectorError` variants (multiple discriminator names may originate from a single variant — e.g. `cursor_decode` / `order_mismatch` / `filter_mismatch` all lift from `UsageCollectorError::Validation`), not from the platform crate.
  - `modkit-odata` — `Page<T>` re-export and OData query parsing (`$filter`, `$orderby`, `$top`, `cursor`), plus `modkit_odata::validate_cursor_against` for cursor binding checks.
  - `types-registry-sdk` — GTS instance and type registry lookups (`TypesRegistryClient::list_instances`) consumed by the host's `GtsPluginSelector` lazily on the first dispatch after the `types-registry` is consistent (single-flight `get_or_init`, cached for the `Service`'s lifetime); binding changes require a module restart.
  - `modkit-security` — `SecurityContext` propagation across REST handlers and the local client.
...
1. **SDK declares the GTS spec**: `usage-collector-sdk/src/gts.rs` declares the unit-struct `UsageCollectorPluginSpecV1` via `#[gts_type_schema(base = PluginV1, schema_id = "gts.cf.modkit.plugins.plugin.v1~cf.core.usage_collector.plugin.v1~", description = "Usage Collector plugin specification", properties = "")]`. The empty `properties = ""` is intentional — instance metadata (`vendor`, `priority`) is carried by the `PluginV1<P>` base type and is not duplicated in the usage-collector-specific spec.
2. **Plugin `init()` publishes the instance**: each `usage-collector-plugin-<backend>` crate's `#[modkit::module]` `init(...)` calls `PluginV1::<UsageCollectorPluginSpecV1>::build_registration("<vendor>.<package>.usage_collector_plugin.v1", cfg.vendor, cfg.priority)?` to assemble the `(instance_id, payload)` pair, registers the payload through `ctx.client_hub().get::<dyn TypesRegistryClient>()?.register(vec![payload]).await?` (gated by `RegisterResult::ensure_all_ok`), and registers the trait object via `ctx.client_hub().register_scoped::<dyn UsageCollectorPluginV1>(ClientScope::gts_id(&instance_id), api)`.
3. **Host resolves and caches the bound instance**: the host's `cpt-cf-usage-collector-component-plugin-host` (in `usage-collector/src/domain/service.rs`) holds a `GtsPluginSelector` that lazily resolves the bound plugin instance — it queries `types-registry` with `UsageCollectorPluginSpecV1::gts_schema_id()` for the pattern `gts.cf.modkit.plugins.plugin.v1~cf.core.usage_collector.plugin.v1~*`, then calls `choose_plugin_instance::<UsageCollectorPluginSpecV1>(&self.vendor, instances)` to pick the lowest-priority match for the configured `[usage_collector].vendor`. The resolved `GtsInstanceId` is cached in the selector for the `Service`'s lifetime.
...
Compile-time linkage is static at the workspace level: plugin crates are built as part of the same Cargo workspace and registered with ModKit via `#[modkit::module]` at startup, but the host `usage-collector` crate depends only on `usage-collector-sdk` and `types-registry-sdk` — never on a concrete `usage-collector-plugin-<backend>` crate. Adding or swapping a plugin is a workspace-build + config-vendor change, not a host-crate change.
