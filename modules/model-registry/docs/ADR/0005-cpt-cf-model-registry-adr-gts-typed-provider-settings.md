...
* `ModelInfoV1<P: gts::GtsSchema = RawProviderSettings>` is declared as the GTS base envelope (`gts.cf.genai.model.info.v1~`).
...
* `RawProviderSettings` is a `#[serde(transparent)]` newtype around `serde_json::Value` with a manual `gts::GtsSchema` impl (empty `SCHEMA_ID`, permissive object schema). It is the default `P` on the SDK's public API (`Model<RawProviderSettings>`).
* `info.gts_type: gts::GtsSchemaId` is the canonical discriminator. Consumers narrow with `Model::try_into_typed::<OpenAiSettingsV1>()`, which checks `info.gts_type == <Target>::SCHEMA_ID` and `serde_json::from_value`s the payload via `gts::GtsDeserializeWrapper`, returning `Result<Model<Q>, ProviderSchemaMismatch>`.
...
* `Model<RawProviderSettings>` is the default public shape returned by `ModelRegistryClientV1`; the previous `Model<AnyProviderSettings>` tagged-enum carrier and its `ProviderKind` discriminant are removed.
* Persistence is one polymorphic JSONB column (`models.provider_settings`) whose shape is identified by the row's `info.gts_type`. On-disk tag and runtime resolution key are identical (`<TypedSettings>::SCHEMA_ID`), so there is no impedance mismatch between the typed Rust view and the JSONB shape.
* Adding a new provider is a kit-level operation: register the new GTS leaf (`gts.cf.genai.model.info.v1~cf.genai._.<vendor>.v1~`); models targeting the new provider keep flowing through the SDK as `RawProviderSettings` until a typed leaf ships.
...
* Typed-narrowing errors change shape: `ProviderKindMismatch` is replaced by `ProviderSchemaMismatch { SchemaId { expected, actual }, Deserialize(serde_json::Error) }` to surface both schema-id mismatches and JSON-shape mismatches separately.

### Confirmation

* Unit tests in `model-registry-sdk/src/models/provider_settings.rs` and `model-registry-sdk/src/models/info.rs` exercise `try_into_typed` resolution by GTS id and the `RawProviderSettings` default carrier path.
...
`ModelInfoV1<P: gts::GtsSchema = RawProviderSettings>` with `#[struct_to_gts_schema]` leaves per provider; discrimination by `info.gts_type`.

* Good, because GTS schema id is the single source of truth — no parallel Rust enum to keep in sync with the on-disk tag.
* Good, because new providers are wired in without an SDK release: unknown `gts_type` values keep flowing as `RawProviderSettings`, and the routing layer can still dispatch on the schema id.
* Good, because the on-disk JSONB tag and the runtime resolution key are the same string (`<P>::SCHEMA_ID`), so there is no translation layer between storage and the SDK.
...
* Bad, because `RawProviderSettings` is a `serde_json::Value` at the SDK boundary; consumers that forget to narrow operate on opaque JSON.
...
* Replaces `AnyProviderSettings` and `ProviderKind` with `RawProviderSettings` (a transparent newtype around `serde_json::Value`) and `ProviderSchemaMismatch`.
...
* `cpt-cf-model-registry-fr-list-tenant-models` — heterogeneous catalog ride on the default `Model<RawProviderSettings>` shape
...
* `cpt-cf-model-registry-component-sdk` — public SDK contract is `Model<RawProviderSettings>` with typed-narrowing helpers
